...
\begin{document}
  ...
  \begin{section}{Reducible cones}\label{sec:reducible cones}
    ...
    \begin{proposition}
      ...
      %
      \begin{proof}
        \cref{itm:condition1} dominates the other two for $m \ge 5$
        and is equivalent to $n \ge \qty{m^2 - 3m + 6}/2$ when $K
        \coloneqq \Lnplus[m]$, so this is a corollary of
        \cref{thm:improved subproblem reduction} in that case. The
        remaining pairs of $m$ and $n$ that satisfy $n \ge \qty{m^2 -
          3m + 6}/2$ but \emph{not} the prerequisites for
        \cref{thm:improved subproblem reduction} are,

        \begin{itemize}
          \begin{item}
            $m = 4$ and $n = 5$, which violates \cref{itm:condition2},
            but leads to no similacra.
          \end{item}
          \begin{item}
            $m = 3$ and $n \in \set{3,4}$, which satisfies neither
            \cref{itm:condition2} nor $n \ge 5$, but still leads to no
            similacra.
          \end{item}
          \begin{item}
            $m = 2$ and $n = 4$, where only $n \ge 5$ is violated but
            the counterexample $\directsum{\Rnplus[2]}{\Lnplus[4]}
            \sim \Hnplus[3]\of{\Rn[1]}$ arises.
          \end{item}
          \begin{item}
            $m = 2$ and $n \in \set{2,3}$, where \cref{itm:condition2}
            and $n \ge 5$ are both violated; there are no similacra.
          \end{item}
          \begin{item}
            $m = 1$ where no similacra arise for $n \in \set{2,3,4}$
            despite $n \ge 5$ being violated and \cref{itm:condition2}
            being violated for $n=2$.
          \end{item}
          \begin{item}
            $m = 0$, which is just \cref{cor:lorentz cone has no
              similacra}.
          \end{item}
        \end{itemize}
        %
        All of these cases are of dimension nine or less, where
        $\Hnplus[3]\of{\Rn[1]}$ is the only non-Lorentz cone that
        would fit, and can be enumerated.
      \end{proof}
    \end{proposition}
