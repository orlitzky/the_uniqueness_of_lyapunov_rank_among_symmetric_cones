  ...
  \begin{lemma}\label{lem:no multiple complex factors}
    ...
    \begin{theorem}\label{thm:subproblem reduction}
      ...
      \begin{proof}
        ...
        other possibility: first we will show that $J$ itself must
        have Lorentz factors, and then \cref{lem:no bigger factors
          pt2,lem:not all smaller factors} will show that at least one
        of those factors is $\Lnplus$.

        Towards the first goal, what we would like to do is show that
        if $J \sim \directsum{\Lnplus}{K}$ consists of various
        non-Lorentz factors, then there exists some symmetric cone $H$
        having $\dim\of{H} = \dim\of{J}$ but $\lyapunovrank{J} <
        \lyapunovrank{H} <
        \lyapunovrank{\directsum{\Lnplus}{K}}$. This of course is a
        contradiction, because then it is impossible for $J$ and
        $\directsum{\Lnplus}{K}$ to share a signature.

        Suppose, for example, that $J \sim \directsum{\Lnplus}{K}$ has
        a factor $\Hnplus[m]\of{\Cn[1]}$ for $m \ge 3$. In other
        words, suppose that $J \cong \Hnplus[m]\of{\Cn[1]} \oplus
        \bar{J}$ where $\bar{J}$ is itself symmetric. With \cref{lem:not
          all smaller factors} in mind, what we would like to do is
        find a set of Lorentz cones, each of dimension strictly less
        than $n$, whose dimensions add up to the dimension of
        ...
        where the last inequality comes from \cref{lem:more
          conditions}. Thus, $m^{2}/2 < n$, and this is an unsubtle
        hint at the dimensions of our Lorentz cones. There are two
        cases for $m$, each of which leads to a different set of
        Lorentz cones:
        %
        \begin{equation*}
          I
        ...
        and the Lorentz factors of $I$ are strictly smaller than $n$.
        We would also like $\lyapunovrank{\Hnplus[m]\of{\Cn[1]}} <
        \lyapunovrank{I}$. A priori this may not be true for all $m$,
        but we can simply check, and $\lyapunovrank{I} - 2m^{2} + 1
        \ge 0$ holds if $m \ge 4$. For $m = 3$, we may take $I
        \coloneqq \Lnplus[9]$ instead of using the formula, since $9 <
        n$ by \cref{itm:condition3}. Now for all $m \ge 3$,
        %
        \begin{equation*}
          \lyapunovrank{J}
          =
        ...
        \end{equation*}
        %
        Define $H \coloneqq \directsum{I}{\bar{J}}$. Does $H$ consist only
        of Lorentz factors? If not, repeat the process above until it
        does: each $\Hnplus[3]\of{\octonions}$ factor can be replaced
        by $I \coloneqq \Lnplus[9] \oplus \Lnplus[9] \oplus
        \Lnplus[9]$, and the other two families can be tackled in the
        same manner as $\Hnplus[m]\of{\Cn[1]}$.

        After a finite number of steps (overwriting the names $I$ and
        $H$ each time), we will be left with $H \coloneqq
        \directsum{I}{\bar{J}}$ where $I$ consists only of Lorentz
        factors whose dimensions are less than $n$, and where
        $\bar{J}$ contains the remaining Lorentz factors that were in
        $J$ when we started. By \cref{lem:no bigger factors pt2}, none
        of the factors in $\bar{J}$ are larger than $\Lnplus$. And if
        the factors of $\bar{J}$ are all smaller than $\Lnplus$, then
        every factor of $H$ is smaller than $\Lnplus$, and
        \cref{lem:not all smaller factors} would show that
        $\lyapunovrank{H} < \directsum{\Lnplus}{K}$. But this of
        course would be a contradiction, since $\lyapunovrank{J} <
        \lyapunovrank{H}$. As a result, at least one of the Lorentz
        factors remaining in $\bar{J}$ is $\Lnplus$, and it came from
        $J$ to begin with.

        Take one of the $\Lnplus$ factors from within $J$, and write
    ...
    It should be clear, on the other hand, that \cref{itm:condition1}
    cannot be weakened without causing problems in \cref{lem:no bigger
      factors pt2}.
  \end{section}
