  ...
  \begin{lemma}\label{lem:no multiple complex factors}
    ...
    \begin{theorem}[Improved \cref{thm:subproblem reduction}]\label{thm:improved subproblem reduction}
      ...
      \begin{proof}
        ...
        4n - 10$ for each $n \in \set{5,6,7,8,9}$. We begin by
        ...
        \end{table}
        %
        The total dimension of $J$ is therefore at most $14$, which
        allows us to systematically eliminate the low-dimensional
        non-Lorentz cones as possible factors of $J$. Note first that
        $\lyapunovrank{\directsum{\Lnplus}{K}} \ge
        \lyapunovrank{\Lnplus} + \dim\of{K}$. If $J$ has an
        $\Hnplus[3]\of{\Rn[1]}$ factor, then $\lyapunovrank{J} = 9 +
        \lyapunovrank{J'} \le 9 + \lyapunovrank{\Lnplus[n + \dim\of{K}
            - 6]}$ by \cref{prop:rank of lorentz is
        ...
        \begin{equation*}
          ...
          -9
          -\lyapunovrank{\Lnplus[n + \dim\of{K} - 6]}
          > 0
        \end{equation*}
        %
        in all cases. Thus the Lyapunov rank of $J$ is not large
        enough to be a similacrum. This rules out
        $\Hnplus[3]\of{\Rn[1]}$ factors. Nor can $J$ have an
        $\Hnplus[4]\of{\Rn[1]}$ factor, because
        $\signature{\Hnplus[4]\of{\Rn[1]}} =
        \signature{\directsum{\Hnplus[3]\of{\Rn[1]}}{\Lnplus[4]}}$,
        and the Lyapunov rank of the latter, as we have just seen, is
        too small. The only other irreducible non-Lorentz factor of
        total dimension less than or equal to $14$ is
        $\Hnplus[3]\of{\Cn[1]}$, and the same gimmick shows that
        %
        \begin{equation*}
          \lyapunovrank{\Lnplus} + \dim\of{K}
          -17
        ...
        \end{equation*}
        %
        although here we check only the pairs $\pair{n}{\dim\of{K}}$
        where $n + \dim\of{K} \ge 9$; that is, where $J$ is big enough
        to hold $\Hnplus[3]\of{\Cn[1]}$. As a result, $J$ has only
        Lorentz factors. The dimension of $K$ is too small to admit
        non-Lorentz factors, so we have Lorentz factors
        everywhere. The remaining cases may therefore be checked by
        enumerating the integer partitions of $\dim\of{K}$ and $n +
        \dim\of{K}$ for all valid combinations of $n$ and $\dim\of{K}$
        as we have done earlier in \cref{tab:HC(3)+L(n) similacra}.
      \end{proof}
    \end{theorem}
