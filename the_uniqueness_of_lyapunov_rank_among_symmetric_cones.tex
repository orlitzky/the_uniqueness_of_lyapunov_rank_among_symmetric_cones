...
\begin{document}
  ...
  \begin{lemma}\label{lem:no multiple complex factors}
    ...
    \begin{conjecture}\label{conj:subproblem reduction}
      ...
      \begin{proof}
        ...
        on $n$. Likewise when $K = \set{0}$.

        In the other direction, and with $\dim\of{K} \ge 1$, we begin
        by recalling \cref{thm:two unique families,ex:reduction is not
          unique}. The cone $J$ is (up to isomorphism) either itself
        of the form
        %
        \begin{equation}\label[form]{eq:lorentz repr}
          \qty{\Hnplus[3]\of{\Cn[1]}}^{j}
          \oplus
          \Lnplus[n_{1}]
          \oplus
        ...
        \end{equation}
        %
        for $j \in \set{0,1}$, or it is a similacrum of such a
        cone. In either case, $J$ shares its signature with one or
        more cones of this form. Suppose $\signature{J} =
        \signature{H}$, where $H$ has \cref{eq:lorentz repr}. We see
        immediately that $H$ contains Lorentz cone factors ($k \ge
        1$); for if not, neither $J = \set{0}$ nor $J \cong
        \Hnplus[3]\of{\Cn[1]}$ could have symmetric similacra. By
        relabeling the factors we may therefore assume as before that
        $n_{1} \ge n_{2} \ge \cdots \ge n_{k}$ in any such $H$. And if
        $n_{1} > n$, then \cref{lem:no bigger factors pt2} would show
        that $\signature{H} = \signature{J} \ne
        \signature{\directsum{\Lnplus}{K}}$. As a result, we have
        $n_{1} \le n$ in any such $H$.

        In what remains we will avoid the question of what happens if
        $n_{1} = n$, and instead suppose that $n_{1} < n$ for all such
        $H$. The goal is to show that $\signature{H} = \signature{J}
        \ne \signature{\directsum{\Lnplus}{K}}$, meaning that our
        supposition cannot hold. To accomplish this, we will again
        analyze cases. In the first two cases we will suppose that $H$
        has no $\Hnplus[3]\of{\Cn[1]}$ factor. The resulting scenario
        follows closely that of \cref{prop:no similacra for h3plus and
          big enough lnplus}, with the new bounds on $n$ supplied by
        \cref{itm:condition1,itm:condition2,itm:condition3}. We
        quickly sketch the reasoning.
        ...
        In what follow, keep in mind that $j$ is the exponent on
        $\Hnplus[3]\of{\Cn[1]}$ in \cref{eq:lorentz repr}, and that
        $n_{1}$ is implicitly (and strictly) bounded above by $n$.

        \begin{pcases}
          \begin{case}[$j=0$ and $n_{1} \le \dim\of{K}$]
            ...
            $n > \dim\of{K}$ to conclude that $\smallestidx < k$. But
            by adding \cref{itm:condition1,itm:condition2} and using
            \cref{itm:condition3} to rule out small $n$, we can derive
            the additional bound $n > 2\dim\of{K} - 1$. Since $K$ can
            be assumed to be nontrivial, we also have $2\dim\of{K} - 1
            \ge \dim\of{K}$, giving $n > \dim\of{K}$. The inequality
            $n > 2\dim\of{K} - 1$ moreover lets us apply
            \cref{impl:growth inequality for f pt2}, and then
            \cref{itm:condition2} lets us conclude that
            $\sum_{i=1}^{k}f\of{n_{i}} < f\of{n} + \lyapunovrank{K}$.
          \end{case}

          \begin{case}[$j=0$ and $n_{1} > \dim\of{K}$]
          ...
          \begin{case}[$j=1$]
            ...
            \end{equation*}
            %
            We would like now to cite the previous two cases, but we
            have to be a little bit careful: the new $\Lnplus[9]$
            factor might be larger than $\Lnplus[n_{1}]$. However,
            \cref{itm:condition3} allows us to say that in any case,
            $9 < n$. We may therefore defer to the previous two cases
            using whichever is larger, $9$ or $n_{1}$, in either case
            concluding that
            %
            \begin{equation*}
              \lyapunovrank{J}
        ...
        We have shown that no $H \sim J$ having \cref{eq:lorentz repr}
        can have a largest $\Lnplus[m]$ factor with $m \ne n$. What
        remains is to show that, if all such $H$ have largest Lorentz
        factor $\Lnplus$, then $J$ itself (up to isomorphism) has an
        $\Lnplus$ factor. This is false if there are no restrictions
        on $n$. For example, $\Hnplus[3]\of{\Rn[1]} \sim
        \directsum{\Lnplus[4]}{\Rnplus[2]}$, but the former is not
        isomorphic to the latter.

        If we drop the condition that $n \ge 10$, then
        $\Hnplus[3]\of{\Rn[1]} \sim
        \directsum{\Lnplus[4]}{\Rnplus[2]}$ is in fact the only
        counterexample. Once $\dim\of{K} \ge 7$ we have $\dim\of{K}
        \le n$, so if we are checking all cases with $n \le 9$, there
        are fewer than $9^{2}$ choices of $\pair{K}{n}$ that satisfy
        \cref{itm:condition1,itm:condition2}.
      \end{proof}
    \end{conjecture}
    ...
    One might wonder whether all three of the conditions on $n$ in
    \cref{conj:subproblem reduction} are strictly necessary. To
    confirm that they are, we demonstrate that each of the three
    conditions can fail even if the other two are met. Let
    $\ell_{1}\of{K}$, $\ell_{2}\of{K}$, and $\ell_{3}\of{K}$ be the
    lower bounds on $n$ given by
    \cref{itm:condition1,itm:condition2,itm:condition3}, respectively.
    %
    \begin{itemize}
      \begin{item}
        If $K = \Lnplus[3]$, then $\ell_{1}\of{K} < \ell_{2}\of{K} <
        \ell_{3}\of{K}$.
      \end{item}
      \begin{item}
        If $K = \Rnplus[5]$, then $\ell_{1}\of{K} < \ell_{3}\of{K} <
        \ell_{2}\of{K}$.
      \end{item}
      \begin{item}
        If $K = \Lnplus[6]$, then $\ell_{2}\of{K} < \ell_{3}\of{K} <
        \ell_{1}\of{K}$.
      \end{item}
    \end{itemize}
  \end{section}
