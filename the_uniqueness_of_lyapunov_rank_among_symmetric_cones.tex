...
\begin{document}
  ...
  \begin{lemma}\label{lem:no multiple complex factors}
    ...
    \begin{example}\label{ex:reduction is not unique}
      ...
      \end{equation*}
      %
      As a result, we can derive a second (non-isomorphic) similacrum
    ...
    find some other similacrum consisting of only Lorentz factors.

    Now since we need only consider Lorentz factors, enumerating the
    possibilities is easily accomplished by partitioning $9+n$ (using,
    for example, the accelerated integer partitioning scheme of
    Kelleher and O'Sullivan~\cite{kelleher-osullivan}). Absent rows
    indicate the absence of similacra.
    %
    \begin{table}[H]
      \begin{center}
        \begin{tabular}{c ccc}
    ...
    left. This is essentially a generalization of... (removed)
    ...
    The $n \ge 15$ bound is not very elegant. Either way we are going
    to wind up checking some low-dimensional cases by hand; this way
    we have more cases to check, but in exchange, a few of the proofs
    are simplified.
