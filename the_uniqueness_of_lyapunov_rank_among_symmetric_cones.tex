  ...
  \begin{lemma}\label{lem:no multiple complex factors}
    ...
    \begin{theorem}[Improved \cref{thm:subproblem reduction}]\label{thm:improved subproblem reduction}
      ...
      \begin{proof}
        ...
        \end{equation*}
        %
        then the Lyapunov rank of $J$ is not large enough to be a
        similacrum. Expanding the expression on the left, we arrive at
        %
        \begin{equation*}
          -\dim\of{K}^{2} + 2(6-\dim\of{K})n + 15\dim\of{K} - 60
          >
          0,
        \end{equation*}
        %
        a strictly increasing function of $n$ for all valid
        $\dim\of{K}$. Substituting $n=5$ and $n=6$, it is thus easy to
        verify. This rules out $\Hnplus[3]\of{\Rn[1]}$. Nor can $J$
        have an $\Hnplus[4]\of{\Rn[1]}$ factor, because
        ...
        too small. The only other non-Lorentz factor of total
        dimension less than or equal to $14$ is
        ...
        \end{equation*}
        %
        As a result, $J$ has only Lorentz factors. The dimension of
        $K$ is too small to admit non-Lorentz factors, so we have
        Lorentz factors everywhere. The remaining cases may therefore
        be checked by enumerating the integer partitions of
        $\dim\of{K}$ and $n + \dim\of{K}$ for all valid combinations
        of $n$ and $\dim\of{K}$ as we have done earlier in
        \cref{tab:HC(3)+L(n) similacra}.
      \end{proof}
    \end{theorem}
  \end{section}
