...
\begin{document}
  ...
  \begin{lemma}\label{lem:no multiple complex factors}
    ...
    \begin{lemma}\label{lem:not all smaller factors}
      ...
      %
      \begin{equation*}
        \signature{\directsummany{i=1}{k}{\Lnplus[n_{i}]}}
        <
        \signature{\directsum{\Lnplus}{K}}
      \end{equation*}
      %
      for any $k \in \Nn[1]$ and $n_{1},n_{2},\ldots,n_{k} < n$.
    ...
    \begin{theorem}\label{thm:subproblem reduction}
      ...
      \begin{proof}
        ...
        and with $\dim\of{K} \ge 1$, our goal is to rule out every
        other possibility: first we will show that $J$ cannot have
        % NOT RIGHT
        non-Lorentz factors, and then \cref{lem:no bigger factors
          pt2,lem:not all smaller factors} will show that at least one
        of those factors is $\Lnplus$.

        Suppose, for example, that $J \sim \directsum{\Lnplus}{K}$ has
        a factor $\Hnplus[m]\of{\Cn[1]}$, for $m \ge 3$. Specifically,
        suppose that $J \cong \Hnplus[m]\of{\Cn[1]} \oplus J'$ for
        some symmetric $J'$. Can we ``replace''
        $\Hnplus[m]\of{\Cn[1]}$ by a sum of Lorentz cones, each having
        dimension strictly less than $n$? Since
        $\Hnplus[m]\of{\Cn[1]}$ is a factor of $J$, and since
        $\dim\of{J} = n + \dim\of{K}$, we can say with certainty that
        %
        \begin{equation*}
          m^{2} = \dim\of{\Hnplus[m]\of{\Cn[1]}} \le n + \dim\of{K} < 2n,
        \end{equation*}
        %
        where the last inequality comes from \cref{lem:more
          conditions}. Thus, $m^{2}/2 < n$. We may consider two cases
        for $m$:
        \begin{equation*}
          H
          \coloneqq
        ...
        \end{equation*}
        %
        In either case, $\dim\of{H} = \dim\of{\Hnplus[m]\of{\Cn[1]}}$,
        and the Lorentz factors of $H$ are strictly smaller than $n$.

        We would also like $\lyapunovrank{\Hnplus[m]\of{\Cn[1]}} <
        \lyapunovrank{H}$. This will not be true for all $m$, but we
        can simply check, and $\lyapunovrank{H} - 2m^{2} + 1 \ge 0$
        holds if $m \ge 4$. And for $m = 3$, we may make the special
        choice $H \coloneqq \Lnplus[9]$ which has $9 < n$ by
        \cref{itm:condition3}. Now for all $m \ge 3$,
        %
        \begin{equation*}
          \lyapunovrank{J}
          =
          \lyapunovrank{\Hnplus[m]\of{\Cn[1]}} + \lyapunovrank{J'}
          <
          \lyapunovrank{H} + \lyapunovrank{J'}.
        \end{equation*}
        %
        If $J'$ now consists only of Lorentz factors, we can invoke
        \cref{lem:no bigger factors pt2,lem:not all smaller factors}
        to conclude two things: none of those factors are bigger than
        $\Lnplus$, and not all of them are smaller. As a result, one would
        have to be $\Lnplus$.

        Of course, $J'$ might have other non-Lorentz factors. But it
        has a finite number of them, and the procedure above can be
        repeated for the other irreducible factors. For each
        $\Hnplus[3]\of{\octonions}$ factor we can use $H \coloneqq
        \Lnplus[9] \oplus \Lnplus[9] \oplus \Lnplus[9]$, and the other
        two families can be tackled in the same manner as
        $\Hnplus[m]\of{\Cn[1]}$.

        % THIS NEXT PARAGRAPH AINT RIGHT
        In the end, we conclude that $J$ has only Lorentz factors, and
        that (at least) one of them is $\Lnplus$. Write $J \cong
        \directsum{\Lnplus}{J'}$. It should be clear that if $J \sim
        \directsum{\Lnplus}{K}$, then the signatures of $J'$ and $K$
        agree. Moreover, $J'$ and $K$ cannot be isomorphic; for if
        they were, then $J$ and $\directsum{\Lnplus}{K}$ would be
        isomorphic, and they are not.
      \end{proof}
    \end{theorem}

    This is false if there are no restrictions on $n$. For example,
    $\Hnplus[3]\of{\Rn[1]} \sim \directsum{\Lnplus[4]}{\Rnplus[2]}$,
    but the former is not isomorphic to the latter. If we drop the
    condition that $n \ge 10$, then $\Hnplus[3]\of{\Rn[1]} \sim
    \directsum{\Lnplus[4]}{\Rnplus[2]}$ is in fact the only
    counterexample. Once $\dim\of{K} \ge 7$ we have $\dim\of{K} \le
    n$, so if we are checking all cases with $n \le 9$, there are
    fewer than $9^{2}$ choices of $\pair{K}{n}$ that satisfy
    ...
    \cref{thm:subproblem reduction} are strictly necessary. To
