  ...
  \begin{lemma}\label{lem:no multiple complex factors}
    ...
    \begin{theorem}[Improved \cref{thm:subproblem reduction}]\label{thm:improved subproblem reduction}
      ...
      \begin{proof}
        ...
        checked by brute force. If we had only Lorentz factors in $K$
        and $J$, we could enumerate the possibilities by partitioning
        the integers $\dim\of{K}$ and $n + \dim\of{K}$ for $K$ and $J$
        respectively. (There are algorithms to do this; for example,
        the accelerated integer partitioning scheme of Kelleher and
        O'Sullivan~\cite{kelleher-osullivan}). But as there may be an
        $\Hnplus[3]\of{\Rn[1]}$ in either cone, the process gets a
        little more complicated.

        For $n \le 9$ a pure integer-partition comparison can be used.
        For each $n \ge 10$ and each associated $\dim\of{K}$ in the
        table, there are a few possibilities. For $K$, we have the
        sums of Lorentz cones that arise from the integer partitions
        of $\dim\of{K}$, but we also have $\Hnplus[3]\of{\Rn[1]}$ in
        $\dim\of{K} = 6$ and
        $\directsum{\Hnplus[3]\of{\Rn[1]}}{\Roneplus}$ in $\dim\of{K}
        = 7$. For $J$, we have the sums of Lorentz cones arising from
        the integer partitions of $n + \dim\of{K}$, but also those
        cones that arise as a direct sum of $\Hnplus[3]\of{\Rn[1]}$
        and an integer partition of $n + \dim\of{K} - 6$. Essentially
        we decide whether or not to include $\Hnplus[3]\of{\Rn[1]}$,
        and then partition the remaining dimensions.
