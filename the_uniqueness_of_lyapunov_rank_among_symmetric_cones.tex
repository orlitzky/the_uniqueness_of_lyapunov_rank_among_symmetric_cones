...
\begin{document}
  ...
  \begin{lemma}\label{lem:no multiple complex factors}
    ...
    \begin{proposition}\label{prop:no similacra for h3plus and big enough lnplus}
      ...
      \begin{proof}
        We first note that it suffices to search for a similacrum of
        the form $\directsummany{i=1}{k}{\Lnplus[n_{i}]}$ with $n-1
        \ge n_{1} \ge n_{2} \ge \cdots \ge n_{k}$ and $k \ge 2$. If a
        similacrum contains $\Hnplus[3]\of{\Cn[1]}$, then, by
        subtracting $\pair{9}{17}$ from both sides, it also contains a
        similacrum for $\Lnplus$, which is not possible by
        \cref{cor:lorentz cone has no similacra}. And if a similacrum
        does not contain $\Hnplus[3]\of{\Cn[1]}$, then either it
        consists of only Lorentz factors, or the results in
        \cref{sec:irreducible cones} let us find some other similacrum
        consisting of only Lorentz factors. Now, assuming that all
        factors are Lorentz, we may reorder them to put the
        superscripts in whatever order we like, and use
        \cref{cor:lorentz cone has no similacra} to conclude that $k
        \ge 2$. Finally, in light of \cref{lem:no bigger factors}, we
        may further assume that $n_{1} \le n-1$, and thus $n_{i} \le
        n-1$ for all $i \in \set{1,2,\ldots,k}$.
    ...
    check the first thirty values of $n$. Since we need only consider
    Lorentz factors, this is easily done by partitioning $9+n$ (using,
    for example, the accelerated integer partitioning scheme of
    Kelleher and O'Sullivan~\cite{kelleher-osullivan}) and computing
    the corresponding Lypaunov ranks. Absent rows indicate the absence
    of similacra.
    %
    \begin{table}[H]
      \begin{center}
    ...
    \begin{lemma}\label{lem:more conditions}
      ...
      2\dim\of{K}$ and $n > \dim\of{K}$.

      \begin{proof}
        The conclusion is obviously true if $K$ is trivial. If $K$ is
        nontrivial, it suffices to show that $n \ge 2\dim\of{K}$. And
        from the assumption that $n \ge 10$, this will obviously hold
        whenever $\dim\of{K} \le 5$.

        Averaging \cref{itm:condition1,itm:condition2}, we obtain
        another lower bound on $n$, a quadratic function of
        $\dim\of{K}$. If this lower bound were itself bounded below by
        $2\dim\of{K}$, then we would be done, but this won't work for
        all values of $\dim\of{K}$. It is however easy to check (by
        solving the quadratic) that $\dim\of{K} \ge 8$ will imply $n
        \ge 2\dim\of{K}$. This leaves only $\dim\of{K} \in \set{6,7}$
        to consider.

        Let $d \in \set{6,7}$. If $K = \Lnplus[d]$, then
        \cref{itm:condition1} will give $n \ge 2\dim\of{K}$. And if $K
        \ne \Lnplus[d]$, then $\lyapunovrank{K} \le \lyapunovrank{J}$
        where $J \coloneqq \directsum{\Lnplus[d-1]}{\Roneplus}$. For
        both $d = 6$ and $d = 7$, one can now easily see that
        %
        \begin{equation*}
          n \ge 2 + \lyapunovrank{\Lnplus[1 + d]} - \lyapunovrank{K}
            \ge 2 + \lyapunovrank{\Lnplus[1 + d]} - \lyapunovrank{J}
            \ge 2d.
        \end{equation*}
        %
        \cref{itm:condition2} therefore gives the desired bound in
        this case.
      \end{proof}
    \end{lemma}
